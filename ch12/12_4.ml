...
(* 目的: ekimei_t型のデータ eki から kana を取得する *)
(* get_kana: ekimei_t -> string *)
let get_kana eki = match eki with
    | {kana = k;} -> k

(* 目的: 予めひらがなの昇順に並んでいるekimei_t型のリスト l とekimei_t型のレコード eki を受け取り、
昇順を保つように eki を l に挿入したときのリストを返す *)
(* insert: ekimei_t list -> ekimei_t -> ekimei_t list *)
let rec insert l eki = match l with
    | [] -> eki :: []
    | first :: rest ->
        if (get_kana eki) <= (get_kana first)
        then eki :: l
        else first :: (insert rest eki)


(* 目的: ekimei_tのリストを受け取り、ひらがなの昇順にソートされたリストを返す *)
(* ins_sort: ekimei_t list -> ekimei_t list *)
let rec ins_sort l = match l with
    | [] -> []
    | first :: rest -> insert (ins_sort rest) first

(* 目的: ひらがなの順にソートされたekimei_t型のリストを受け取り、駅の重複を取り除き、ekimei_t型のリストを返す *)
(* remove_dup: ekimei_t list -> ekimei_t list *)
let rec remove_dup l = match l with
    | [] -> []
    | f1 :: r1 -> match r1 with
        | [] -> [f1]
        | f2 :: r2 ->
            if (get_kana f1) = (get_kana f2)
            then remove_dup (f1 :: r2)
            else f1 :: remove_dup r1


(* 目的: ekimei_t型のリストを受け取り、それをひらがなの順に整列し、さらに駅の重複を取り除いた
ekimei_t型のリストを返す *)
(* seiretsu: ekimei_t list -> ekimei_t list *)
let seiretsu l =
    let sorted = ins_sort l in
    remove_dup sorted
