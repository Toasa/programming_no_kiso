...
let rec make_eki_list l = match l with
    | [] -> []
    | {kanji = k;} :: rest ->
        {name = k; min_dis = infinity; temae_list = []} :: make_eki_list rest

let eki_t_list = make_eki_list global_ekimei_list


(* 目的: eki_t型のリストと起点（漢字の文字列）を受け取り、起点を初期化し、eki_t型のリストを返す *)
(* shokika: eki_t list -> string -> eki_t list *)
let rec shokika l eki_name = match l with
    | [] -> []
    | first :: rest -> match first with
      | {name = n; min_dis = min; temae_list = t} ->
        if n = eki_name
        then {name = n; min_dis = 0.; temae_list = [n]} :: rest
        else first :: (shokika rest eki_name)

let test = shokika eki_t_list "神田"
