...
(* 目的: ローマ字の駅名を２つ受け取り、その間の距離を調べ、直接つながっている場合は
「A駅からB駅まではXXXkmです」という文字列を返し、つながっていない場合は
「A駅からB駅はつながっていません」という文字列を返す *)
(* kyori_wo_hyoji: string -> string -> string *)
let kyori_wo_hyoji eki1_r eki2_r =
    let eki1_kan = romaji_to_kanji eki1_r global_ekimei_list in
    let eki2_kan = romaji_to_kanji eki2_r global_ekimei_list in
    let dis = get_ekikan_kyori eki1_kan eki2_kan global_ekikan_list in
    if dis <> infinity then eki1_kan ^ "駅から" ^ eki2_kan ^ "駅までは" ^ string_of_float dis ^"kmです"
    else eki1_kan ^ "駅から" ^ eki2_kan ^ "駅はつながっていません"

let test1 = kyori_wo_hyoji "kiyosumi-shirakawa" "sumiyoshi" = "清澄白河駅から住吉駅までは1.9kmです"
let test2 = kyori_wo_hyoji "yotsuya" "komagome" = "四ツ谷駅から駒込駅はつながっていません"
