...
(* 目的: ekimei_tのデータを受け取り、eki_t型のデータを返す *)
(* make_eki: ekimei_t -> eki_t *)
let make_eki e = match e with
    | {kanji = k} -> {name = k; min_dis = infinity; temae_list = []}

(* 目的: ekimei_tのリストを受け取ると、eki_t型のリストを生成する *)
(* make_eki_list: ekimei_t list -> eki_t list *)
let make_eki_list l = List.map make_eki l

(* 目的: eki_t型のリストと起点（漢字の文字列）を受け取り、起点を初期化し、eki_t型のリストを返す *)
(* shokika: eki_t list -> string -> eki_t list *)
let shokika l eki_name = List.map
    (fun eki -> if eki.name = eki_name
                then {name = eki_name; min_dis = 0.; temae_list = [eki_name]}
                else eki)
    l

let test_eki_t_list = make_eki_list global_ekimei_list
let test = shokika test_eki_t_list "浅草"
