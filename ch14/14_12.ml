...
(* 目的: ekimei_t型のリストと起点となる駅の漢字名を受け取り、eki_t型のリストを生成し、
その中の起点を初期化する *)
(* make_initial_eki_list: ekimei_t list -> string -> eki_t list *)
let make_initial_eki_list ekimei_t_list kiten =
    let eki_t_list = List.map make_eki ekimei_t_list in
    List.map
    (fun eki -> if eki.name = kiten
                then {name = kiten; min_dis = 0.; temae_list = [kiten]}
                else eki) eki_t_list

let test = make_initial_eki_list global_ekimei_list "浅草"
